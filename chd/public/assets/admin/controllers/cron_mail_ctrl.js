"use strict";
app.controller('cronMailCtrl', ['$scope', '$http', '$rootScope', '$location', 'AlertService', 'RestSvr',
	function($scope, $http, $rootScope, $location, AlertService, RestSvr) {
		...
		var load = function (isCache, sortBy, queryEmail) {
			...
			RestSvr.paginate('get_search_alerts','','page=' + $scope.paging.page + qEmail + sort, {cache: isCache}).then(function(response){
				$scope.spinner = false;
				if(response.errors){
				...
				{
					$scope.cronmails = response.records;
